...
WALL_ITERATIONS        = 300
WALL_PASSES            = 4      # max number of wall planes to remove
DBSCAN_EPS             = 0.4
DBSCAN_MIN_PTS         = 10
CLUSTER_MIN_SIZE       = 5
...
# ---------------------------------------------------------------------------
# PCA Analysis (for shape recognition)
# ---------------------------------------------------------------------------

def analyze_cluster_pca(cluster_pts: np.ndarray):
...
def is_vertical_pole(cluster_pts: np.ndarray):
    eigvals, eigvecs = analyze_cluster_pca(cluster_pts)

    v1 = eigvecs[:, 0]
    lambda1, lambda2, lambda3 = eigvals

    # ===== 判别指标 =====
    vertical_score = abs(v1[2])                 # 是否竖直
    linear_ratio   = lambda1 / (lambda2 + 1e-6) # 是否细长
    shape_ratio    = lambda2 / (lambda3 + 1e-6) # 是否对称
    ...
    is_pole = (
        vertical_score > 0.85 and
        linear_ratio   > 5.0 and
        shape_ratio    < 2.0 and
        height > 0.5 and
        width  < 0.5
    )
...
class IndustrialLidarNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._attitude_calibrated = False
        self._q_offset = None
    ...
    def vehicle_attitude_callback(self, msg: VehicleAttitude) -> None:
        q_meas = np.array(msg.q, dtype=np.float64)

        # =========================
        # STEP 1: 初始化标定（只做一次）
        # =========================
        if not self._attitude_calibrated:
            q_true = np.array([0.7071, 0.0, 0.0, 0.7071])

            q_inv = quat_inverse(q_meas)

            # Δq = q_true * q_meas^-1
            self._q_offset = quat_multiply(q_true, q_inv)

            self._attitude_calibrated = True

            
            self.get_logger().info(f"q_measured = {q_meas}")
            self.get_logger().info(f"q_offset   = {self._q_offset}")
            self.get_logger().info("=== Attitude Calibration DONE ===")

        # =========================
        # STEP 2: 应用校准
        # =========================
        if self._q_offset is not None:
            q_corrected = quat_multiply(self._q_offset, q_meas)
        else:
            q_corrected = q_meas

        # =========================
        # STEP 3: 重新封装为 VehicleAttitude
        # =========================
        ...
        # STEP 4: 覆盖原变量（关键！）
        # =========================
        self._vehicle_attitude = corrected_msg
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def _callback(self, msg: PointCloud2) -> None:  # noqa: C901
        ...
        for idx, lab in enumerate(valid_labels, 1):
            ...
            # ================= 2. PCA分析 =================
            is_pole_flag, pca_info = is_vertical_pole(cluster_pts)

            self.get_logger().info(
                f"[PCA] λ1/λ2={pca_info['linear_ratio']:.2f} | "
                f"λ2/λ3={pca_info['shape_ratio']:.2f} | "
                f"vertical={pca_info['vertical_score']:.2f}"
            )

            if is_pole_flag:
                self.get_logger().info(">>> DETECTED VERTICAL POLE <<<")
            ...
            if enu_available:
                ...
                marker_x, marker_y, marker_z = center_enu
                marker_frame = "map"
            else:
                clusters_enu.append(center_flu.copy())
                marker_x, marker_y, marker_z = center_flu
                marker_frame = msg.header.frame_id

            # ================= 4. 打印信息 =================
            self.get_logger().info(
                f"[CLUSTER_{idx}] ID={int(lab)} | Points={len(cluster_pts)}"
            )
            self.get_logger().info(
            ...
            bbox_m.pose.position.x    = float(marker_x)
            bbox_m.pose.position.y    = float(marker_y)
            bbox_m.pose.position.z    = float(marker_z)
            ...
            if is_pole_flag:
                bbox_m.color.r = 1.0
                bbox_m.color.g = 0.0
                bbox_m.color.b = 0.0   # 🔴 柱子
            else:
                bbox_m.color.r = 0.9
                bbox_m.color.g = 0.9
                bbox_m.color.b = 0.1   # 🟡 普通物体
